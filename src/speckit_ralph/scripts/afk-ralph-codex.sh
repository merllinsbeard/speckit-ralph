...
log_activity "LOOP START run=$RUN_ID iterations=$ITERATIONS cli=codex"
...
for ((i=1; i<=ITERATIONS; i++)); do
  ...
  append_run_summary "$ITER_START_FMT | run=$RUN_ID | iter=$i | cli=codex | duration=${ITER_DURATION}s | status=$STATUS_LABEL"
